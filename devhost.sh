...
save_json() {
    tmpfile="$(mktemp "${CONFIG}.tmp.XXXXXX")" || return 1
    printf '%s' "$1" > "$tmpfile"
    mv "$tmpfile" "$CONFIG"
}
...
validate_name() {
    ...
    if [ ${#name} -gt 63 ]; then
        echo "Name too long (max 63 chars)."
        return 1
    fi
...
Commands:
  ...
  hosts <sync|clear>      Windows-only hosts management (use devhost.ps1)
...
add() {
    ...
    if [[ "$target" =~ ^https?:// ]]; then
        ...
        if [[ "$stripped" == *":"* ]]; then
            host_part="${stripped%:*}"
            port_part="${stripped##*:}"
            if [ -z "$host_part" ] || ! validate_port_range "$port_part"; then
                echo "Port must be a number or host:port"
                return 1
            fi
        else
            if ! validate_port_range "$stripped"; then
                echo "Port must be a number or host:port"
                return 1
            fi
        fi
        ...
        if ! validate_port_range "$target"; then
            echo "Port must be a number or host:port"
            return 1
        fi
    fi
...
hosts_cmd() {
    echo "Hosts management is supported on Windows via devhost.ps1."
    echo "Usage: devhost.ps1 hosts sync|clear"
    return 1
}
