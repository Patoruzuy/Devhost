...
def check_key_permissions(key_path: Path) -> tuple[bool, str]:
    ...
    try:
        ...
        if mode & (stat.S_IRGRP | stat.S_IWGRP):
            logger.warning(f"Private key {key_path} is group-readable/writable (permissions: {oct(stat.S_IMODE(mode))}). Consider setting to 0600.")
...
def check_certificate_expiration(cert_path: Path, warning_days: int = 30) -> tuple[bool, Optional[datetime], str]:
    ...
    try:
        ...
        try:
            ...
            if days_until_expiry < 0:
                return True, expiration_date, f"Certificate {cert_path.name} EXPIRED on {expiration_date.strftime('%Y-%m-%d')}"
            elif days_until_expiry <= warning_days:
                return True, expiration_date, f"Certificate {cert_path.name} expires in {days_until_expiry} days ({expiration_date.strftime('%Y-%m-%d')})"
            else:
                return False, expiration_date, f"Certificate {cert_path.name} valid until {expiration_date.strftime('%Y-%m-%d')} ({days_until_expiry} days remaining)"
            ...
            return False, None, f"Cannot check expiration for {cert_path.name} (install cryptography package or openssl for full validation)"
