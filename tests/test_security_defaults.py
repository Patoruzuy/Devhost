...
class TestSecureDefaults(unittest.TestCase):
    ...
    def test_default_bind_address_is_localhost(self):
        ...
        create_app()  # Just verify it can be created
        ...
        if default_host:
            self.assertIn(
                default_host, {"127.0.0.1", "localhost"}, f"Default bind address must be localhost, got: {default_host}"
            )
    ...
    def test_ipv6_literal_loopback_allowed(self):
        """Allow bracketed IPv6 loopback literals (e.g., [::1])."""
        if not socket.has_ipv6:
            self.skipTest("IPv6 not available in this environment")
        from devhost_cli.router import security

        is_valid, error_msg = security.validate_upstream_target("[::1]", 8000)
        self.assertTrue(is_valid, f"IPv6 loopback should be allowed: {error_msg}")

    def test_ipv6_mapped_private_blocked(self):
        """Block IPv4-mapped IPv6 addresses that target private ranges."""
        if not socket.has_ipv6:
            self.skipTest("IPv6 not available in this environment")
        from devhost_cli.router import security

        is_valid, error_msg = security.validate_upstream_target("::ffff:10.0.0.1", 8000)
        self.assertFalse(is_valid, "IPv4-mapped private address should be blocked")
        if error_msg:
            self.assertIn("IPv4-mapped", error_msg)
    ...
    def test_metadata_endpoints_blocked(self):
        ...
        for host, port in metadata_hosts:
            ...
            self.assertFalse(is_valid, f"Metadata endpoint should be blocked: {host}:{port}")
            self.assertIn("metadata", error_msg.lower(), f"Error message should mention metadata: {error_msg}")
    ...
    def test_only_http_https_schemes_allowed(self):
        ...
        for target, expected in valid_targets:
            ...
            self.assertEqual(result, expected, f"Valid target {target} should parse correctly")
        ...
        for target in invalid_targets:
            result = parse_target(target)
            self.assertIsNone(result, f"Dangerous scheme should be rejected: {target}")
    ...
    def test_hostname_validation_blocks_control_characters(self):
        ...
        for hostname in valid_hostnames:
            ...
            self.assertTrue(is_valid, f"Valid hostname {hostname} should pass validation: {error_msg}")
        ...
        for hostname in invalid_hostnames:
            is_valid, error_msg = validate_hostname(hostname)
            self.assertFalse(is_valid, f"Invalid hostname should be rejected: {repr(hostname)}")
    ...
    def test_default_timeout_is_reasonable(self):
        ...
        self.assertGreaterEqual(default_timeout, 30, "Default timeout should be at least 30 seconds")
        self.assertLessEqual(default_timeout, 120, "Default timeout should not exceed 120 seconds")
    ...
    def test_environment_variable_overrides_require_explicit_opt_in(self):
        ...
        self.assertTrue(is_valid, f"DEVHOST_ALLOW_PRIVATE_NETWORKS=1 should allow private IPs: {error_msg}")
    ...
    def test_state_file_not_world_readable(self):
        ...
        if state_file.exists():
            ...
            self.assertFalse(
                group_read or other_read, f"state.yml should not be world-readable (permissions: {oct(perms)})"
            )
    ...
    def test_logging_disabled_by_default(self):
        ...
        self.assertEqual(log_requests, "0", "Request logging should be disabled by default to prevent secret leakage")
    ...
    def test_no_hardcoded_secrets_in_defaults(self):
        ...
        for pattern in suspicious_patterns:
            ...
            self.assertFalse(hasattr(default_config, pattern), f"Config should not have hardcoded {pattern}")
...
class TestWindowsSecurityDefaults(unittest.TestCase):
    ...
    def test_hosts_file_operations_require_admin(self):
        ...
        self.assertIsInstance(admin_status, bool, "is_admin() should return a boolean")
    ...
    def test_hosts_file_backup_created_before_modification(self):
        ...
        # Should be callable
        self.assertTrue(callable(hosts_backup), "hosts_backup() should be a callable function")
